...
User flow to upgrade SONiC extention:
1. Upgrade a feature
...
Commands:
  ...
  upgrade     Upgrade SONiC extention
...
```
admin@sonic:~$ # Pull latest compatible feature version
admin@sonic:~$ sudo sonic-extention upgrade featureA
admin@sonic:~$ # Upgrade to specific tag
admin@sonic:~$ sudo sonic-extention upgrade featureA --tag=latest
admin@sonic:~$ # Enable warm restart if feature supports
...
SONiC application extention has to provide a CLI/Rest/Klish CLI/etc. interfaces. In order to avoid extra dependency on a specific click version or other interface implementation details, the code for those interfaces will be autogenerated based on extention provided definitios.
...
See, below on what SONiC application extention must provide and what is optional.

## Manifest schema definition

SONiC application extention **must** provide a *manifest.json* file.
...
/implicit_requires | list of strings | List of SONiC core service the application requires implicitely (on cold starts/stops)
/warm_stop_stage| string | When to stop service on warm shutdown only one stage for know - "syncd"
/fast_stop_stage| string | Fast to stop service on fast shutdown only one stage for know - "syncd"
...
SONiC application extentions **can** provide a CONFIG DB schema definiton file.
...
SONiC application extentions **can** provide *additional_commands.json* file.
...
Only, "get" or "del" types are allowed here, which will map to CLI show and sonic-clear respectively. "set" and "mod" are config commands, therefore covered in schema.json file.
...
/processes/{{index}}/critical | bool | Wether as process is defined as critical
...
anything-else | Specific version, version *may* not contain SONiC release number

If realse version is installed it will be possible to lookup the tag lable in image metadata.

# Fast/Warm-reboot

A SONiC extention *must* not break SONiC fast/warm reboot/restart functionality.
The way a SONiC extention is aware about fast/warm reboot/restart intention is through the CONFIG DB flag.

There could be two possibilities:
1. A service requires swss. In order to not break fast/warm reboot this service *must* be explicitely stopped before swss is stopped.
2. A service requires swss implicitely. In this case, a feature can do warm restart independently from swss as well as swss can do warm restart independently from the feature service. A warm_stop_stage/fast_stop_stage control the stage when a service should be stopped. Otherwise it is killed at the end of warm shutdown sequence.

# Debugubility

A SONiC extention *can* follow a convention, that ```show featureA dump``` will generate a feature dump to stdout. If command exists, ```show techsupport``` will save the dump into a file.

# Upgrade

During SONiC-2-SONiC uprade a feature extention will be upgraded to the latest matching version as well. 
